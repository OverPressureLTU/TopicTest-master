...
public class Train_PlayerCtrl : MonoBehaviour
{
    #region public
    ...
    void FixedUpdate()
    {
        if (isWater)
        {
            ...
            train.btnWater.interactable = false;
        }
        else
        {
            train.btnWater.interactable = true;
        }
        if (isIce)
        {
            IceType();
            animator.SetBool("IceType", true);
            train.btnIce.interactable = false;
        }
        else
        {
            train.btnIce.interactable = true;
        }
        if (isSteam)
        {
            SteamType();
            animator.SetBool("SteamType", true);
            train.btnSteam.interactable = false;
        }
        else
        {
            train.btnSteam.interactable = true;
        }
    }
    ...
    public void WaterType() //水型態
    {
        ...
        if (Input.GetKey(KeyCode.A))    //按A向左移動
        {
            ...
            {
                if (train.slidStamina.value < 10)
                {
                    return;
                }
                train.slidStamina.value -= 10;
